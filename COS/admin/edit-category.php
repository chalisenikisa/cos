...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) redirect('manage-categories.php');

$stmt = $pdo->prepare("SELECT * FROM categories WHERE id = ?");
$stmt->execute([$id]);
$category = $stmt->fetch();
if (!$category) redirect('manage-categories.php');
...
<head>
    ...
    <title>Edit Category — Canteen Food Ordering Admin</title>
...
<div class="admin-layout">
    ...
    <main class="admin-content">
        <h1 style="font-family:var(--font-display);font-size:1.8rem;margin-bottom:24px;">Edit Category</h1>
        ...
        <div class="admin-form-card">
            ...
            <form method="POST" action="">
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" value="<?= sanitize($category['name']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="icon">Icon (optional)</label>
                    <input type="text" id="icon" name="icon" value="<?= sanitize($category['icon']) ?>">
                </div>
